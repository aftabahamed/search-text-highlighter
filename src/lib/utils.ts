...
const Utils: UtilsType = {
  validate: {
    ...
    options(options: OptionsType = {}): void {
      ...
      if (
        typeof options.customClass !== 'undefined' &&
        typeof options.customClass !== 'string'
      ) {
        throw new Error('The customClass option should be a string.');
      }
      if (
  ...
  getOptions(options: OptionsType = {}): OptionsType {
    ...
    return {
      ...
      customClass: options.customClass ? options.customClass : 'text-highlight',
      matchAll:
